...
/// For the thermistor / PTC measuring the lid's temperature
...
/// For the thermistor / PTC measuring the metal block's temperature
...
float TableLookup(
  const unsigned long lookupTable[],
...
///Find the index value of a searh value, with startValue added
/// lookupTable[0] = startValue
///startValue: the mapped value at index 0,
///for example, for the PLATE_RESISTANCE_TABLE lookupTable,
///startValue equals -40, so lookupTable[0] equals -40 ?degrees Celsius?
float TableLookup(
...
void CLidThermistor::ReadTemp() {
  unsigned long voltage_mv = (unsigned long)analogRead(m_pin) * 5000 / 1024;
  unsigned long resistance = voltage_mv * 2200 / (5000 - voltage_mv);
  
  iTemp = TableLookup(
    LID_RESISTANCE_TABLE,
    sizeof(LID_RESISTANCE_TABLE) / sizeof(LID_RESISTANCE_TABLE[0]),
    0,
    resistance);
}
    ...
    m_pin(4)
{
  ...
  //spi setup
  //SPI = Serial Periheral Interface bus
...
void CPlateThermistor::ReadTemp()
{
  ...
  const double max_voltage = 5.0; //Volts
  const double max_voltage_mv = 1000.0 * max_voltage; //Millivolts
  const double resistor = 2200.0; //Ohm, the resistor in series with the PTC
  //If T ==   0 K, U = 5.0 V
  //If T == inf K, U = 0.0 V

  //Should be read from pin #4 instead
  const int raw_input = analogRead(m_pin);
  const double f_input
    = static_cast<double>(raw_input)
    / static_cast<double>(max_input);
  const unsigned long voltage_mv
    = static_cast<unsigned long>(
      f_input * max_voltage_mv);
  const unsigned long resistance
    = voltage_mv * resistor / (max_voltage_mv - voltage_mv);
